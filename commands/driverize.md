---
description: Install Driver enforcement stack — hooks, shadow agents, context injection, and CLAUDE.md routing
argument-hint: ""
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, mcp__driver-mcp__get_codebase_names
---
...
Create backups of files that will be modified. Only create the `.pre-driver` backup if no `.pre-driver` file already exists at that path. If `.pre-driver` already exists, the true original is already preserved — do not overwrite it.

- `CLAUDE.md` → `CLAUDE.md.pre-driver` (if CLAUDE.md exists and `CLAUDE.md.pre-driver` does not)
- `.claude/settings.json` → `.claude/settings.json.pre-driver` (if settings.json exists and `.claude/settings.json.pre-driver` does not)
...
### 2.5: Detect Existing Installation

Check for an existing Driverize installation by looking for version stamps in the artifacts read during Phase 1:

1. **settings.json**: Look for the `_driverize` key. If present, read `_driverize.version` to get the installed version.
2. **CLAUDE.md**: Look for `<!-- driverize:v` comment marker. Extract the version number from the marker.
3. **Hook scripts** (`.claude/hooks/`): Look for `# Driverize v` comment lines. Extract the version number.

**If version stamps are found:**
- Report the installed version: "Driverize v{installed} is already installed."
- Compare with current `DRIVERIZE_VERSION`. If same version: "Re-running will regenerate all artifacts." If different version: "Upgrading from v{installed} to v{current}."
- Proceed with installation (Phase 3+) — the re-run will overwrite artifacts with current-version stamps.

**If no version stamps are found but Driverize artifacts exist** (hook files, shadow agents, Driver block in CLAUDE.md without markers):
- Report: "Existing Driverize installation detected without version stamps (pre-versioning install). Re-running will add version stamps to all artifacts."
- Proceed with installation.

**If no Driverize artifacts are found:**
- Report: "No existing Driverize installation detected. Proceeding with fresh install."
...
```bash
#!/usr/bin/env bash
# Driverize v__DRIVERIZE_VERSION__ — generated by driverize prompt
# Do not edit manually; re-run driverize to update
...
### To rollback:
Run `/drvr:un-driverize` to cleanly reverse all changes (or use the manual commands below):
...
- **Substitute all placeholders** — `__MCP_SERVER_NAME__`, `__DRIVER_VERIFY_TOKEN__`, `__DRIVERIZE_VERSION__`, and `__TIMESTAMP__` must be replaced before writing
