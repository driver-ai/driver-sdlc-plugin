...
## Code Self-Sufficiency
The code you write must stand on its own. Someone reading only the source — with no access to this prompt, the plan, the task docs, or any process artifact — must be able to understand it.
...
9. Report: what you built, files touched, any deviations from the spec (especially core/shell boundary deviations), and any standards compliance concerns
```
